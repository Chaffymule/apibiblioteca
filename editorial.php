...
class editorial
{
    ...
    private static function obtenerEditorial($id_editorial)
    {
        try {
            ...
            if (!$editorial) {
                throw new ExcepcionApi(self::ESTADO_NO_ENCONTRADO, "El botón con ID $id_editorial no existe", 404);
            }
    ...
    public static function delete($peticion)
    {
        ...
        if (!empty($peticion)) {
            ...
            if (count($peticion) == 2) {
                ...
                $idBoton = $peticion[0];
                return self::eliminarEditorial($idBoton);
            }
        } else {
            throw new ExcepcionApi(self::ESTADO_URL_INCORRECTA, "URL mal formada", 400);
        }
    ...
    public static function put($peticion)
    {
        if (!empty($peticion)) {
            ...
            $cuerpo = file_get_contents('php://input');
            $datosEditorial = json_decode($cuerpo);
            return self::modificarEditorial($id_editorial, $datosEditorial);
        } else {
            throw new ExcepcionApi(self::ESTADO_URL_INCORRECTA, "URL mal formada", 400);
        }
    }

    public static function modificarEditorial($id_editorial, $datosEditorial)
    {
        $nombre = $datosEditorial->nombre;
        $direccion = $datosEditorial->direccion;
        ...
        try {
            ...
            $sentencia->bindParam(1, $nombre);
            $sentencia->bindParam(2, $direccion);
            $sentencia->bindParam(3, $id_editorial);
